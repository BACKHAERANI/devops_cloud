...
def insertDate():
    ...
    sql = ""
    sql = "INSERT INTO raspberrypyTBL( name, birth, gender, posi, score) VALUES ('" + name + "'," + birth + ",'" + gender + "','" + posi + "',"+score+")"
...
def deleteData():
    ...
    name = listname.get(listname.curselection())  # 튜플변환


    sql = "DELETE FROM raspberrypytbl WHERE name = '"+name+"'"

    answer = messagebox.askokcancel("경고", "연습생 정보를 삭제하시겠습니까?")    #OK. CANCEL 명령사용
...
fileMenu.add_command(label="예상 데뷔조", command=func_open)
